...
const AdminDashboard = () => {
    ...
    const [formData, setFormData] = useState({
        title: '', description: '', date: '', location: '', category: '', totalSeats: '', ticketPrice: '', imageUrl: '' // ✅ Fixed: was 'image'
    });
    ...
    const fetchData = async () => {
        try {
            const [eventsRes, bookingsRes] = await Promise.all([
                api.get('/events'),
                api.get('/bookings/all') // ✅ Fixed: was '/bookings/my' which only returns current user's bookings
            ]);
    ...
    const handleCreateEvent = async (e) => {
        e.preventDefault();
        try {
            await api.post('/events/create', formData); // ✅ Fixed: was '/events' which hits getAllEvents
            setShowEventForm(false);
            setFormData({ title: '', description: '', date: '', location: '', category: '', totalSeats: '', ticketPrice: '', imageUrl: '' }); // ✅ Fixed: was 'image'
            fetchData();
        } catch (error) {
                                ...
                                bookings.map(booking => (
                                    <li key={booking._id} className={`p-6 hover:bg-gray-50 transition border-l-4 ${booking.status === 'pending' ? 'border-l-yellow-400' : booking.status === 'confirmed' ? 'border-l-green-400' : 'border-l-red-400'}`}>
                                        <div className="flex justify-between items-start mb-3">
                                            {/* ✅ Fixed: was booking.eventId, schema uses eventID */}
                                            ...
                                            <div className="flex flex-col gap-1 items-end shrink-0 ml-4">
                                                ...
                                                {booking.status !== 'cancelled' && <span className={`px-2 py-1 text-[10px] font-black rounded uppercase tracking-wider ${booking.paymentStatus === 'paid' ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-200 text-gray-800'}`}>{booking.paymentStatus}</span>}
                                            </div>
                                        </div>
                                        <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-100 text-sm">
                                            ...
                                            <p className="text-gray-700 flex items-center gap-2 mb-1">
                                                ...
                                                {/* ✅ Fixed: was booking.bookedAt, schema uses createdAt via timestamps */}
                                        ...
                                        {booking.status === 'pending' && (
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                ...
                                                {/* ✅ Fixed: was 'not_paid' which schema doesn't allow, changed to 'unpaid' */}
